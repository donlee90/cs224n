    ...
    ### YOUR CODE HERE (5-20 lines)
        for idx, word in enumerate(sentence):
            ...
            for w in range(-window_size, window_size + 1):
                if (idx+w) < 0:
                    window += start
                elif (idx+w) > len(sentence)-1:
                    window += end
                else:
                    window += sentence[idx+w] 
            windowed_data.append((window,labels[idx]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_window_features))
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_tensor = tf.Variable(self.pretrained_embeddings) # embedding matrix.
        embeddings_pre_concatenate = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder) # tensor of shape (None, n_window_features, embedding_size)
        embeddings = tf.reshape(embeddings_pre_concatenate, (-1, self.config.n_window_features*self.config.embed_size)) 
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable("b1", (self.config.hidden_size,), tf.float32, tf.contrib.layers.xavier_initializer())
        ...
        b2 = tf.get_variable("b2", (self.config.n_classes,), tf.float32, tf.contrib.layers.xavier_initializer())
