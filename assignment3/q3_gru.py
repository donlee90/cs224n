...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        output, _ = tf.nn.dynamic_rnn(cell, x, None, tf.zeros((tf.shape(x)[0], cell.state_size),tf.float32))
        preds = tf.sigmoid(tf.reshape(output[:,self.config.max_length-1,:],(-1,1)))
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.l2_loss(y-preds)) #BK: might wanna use tf.ls_loss if it doesn't work.
        ### END YOUR CODE
